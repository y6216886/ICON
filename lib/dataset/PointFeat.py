...
class PointFeat:
    ...
    def query(self, points, feats={}, bs_idx=None):
        """
        ...
        for feat_key in feats.keys():
            ...
            if feat_key in del_keys:
                ...
                if bs_idx!=None:
                    feat_arr = feats[feat_key][bs_idx][None,...]
                else:
        ...
        if "sdf" in out_dict.keys(): ###add noise to it 
            ...
            pts_signs = 2.0 * (check_sign(self.verts, self.faces[0], points).float() - 0.5) ###问题出在这
        ...
        for out_key in out_dict.keys():
            out_dict[out_key] = out_dict[out_key].view(self.Bsize, -1, out_dict[out_key].shape[-1]) ##here makes 8000 to 4000

        return out_dict

class PointFeat_grad:
    def __init__(self, verts, faces, args=None):

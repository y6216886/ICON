...
from lib.net.lightweight_config import INPLANES, NUM_FEATS, EXPANSION, SEPARABLE_ALL, SEPARABLE_3x3, CONCAT, PERCEPTUAL_RES, PERCEPTUAL_LOSS
...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=6, first_conv_channel=3):
        super(HourglassNet, self).__init__()
        
        self.inplanes = INPLANES  # Edit from 64
        self.num_feats = NUM_FEATS # Edit from 128
        self.num_stacks = num_stacks
        self.conv1 = nn.Conv2d(first_conv_channel, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=True)
...
def hg(**kwargs):
    model = HourglassNet(Bottleneck, num_stacks=kwargs['num_stacks'], num_blocks=kwargs['num_blocks'],
                         num_classes=kwargs['num_classes'], first_conv_channel=kwargs['first_conv_channel'])
    return model
...
def hg1(pretrained=False, progress=True, num_blocks=1, num_classes=6, first_conv_channel=3):
    return _hg('hg1', pretrained, progress, num_stacks=1, num_blocks=num_blocks,
               num_classes=num_classes, first_conv_channel=first_conv_channel)


def hg2(pretrained=False, progress=True, num_blocks=1, num_classes=16, separable=False, first_conv_channel=3):
    return _hg('hg2', pretrained, progress, num_stacks=2, num_blocks=num_blocks,
               num_classes=num_classes, separable=separable, first_conv_channel=first_conv_channel)


def hg3(pretrained=False, progress=True, num_blocks=1, num_classes=16, separable=False, first_conv_channel=3):
    return _hg('hg3', pretrained, progress, num_stacks=3, num_blocks=num_blocks,
               num_classes=num_classes, first_conv_channel=first_conv_channel)

def hg8(pretrained=False, progress=True, num_blocks=1, num_classes=16, first_conv_channel=3):
...
if __name__ == "__main__":
    ...
    net=hg1(pretrained=False, first_conv_channel=6).cuda() #1306MiB
    # net=hg2(pretrained=False, first_conv_channel=6).cuda() #1306MiB
    # net=hg3(pretrained=False, first_conv_channel=6).cuda() #1306MiB
    # net=hg8(pretrained=False, first_conv_channel=6).cuda() #1306MiB
    a=net(x)
    print(1)